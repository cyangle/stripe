...
module Stripe
  ...
  class PaymentMethodDetailsCardPresentReceipt
    ...
    ENUM_VALIDATOR_FOR_ACCOUNT_TYPE = OpenApi::EnumValidator.new("account_type", "String", ["checking", "credit", "prepaid", "unknown"])
    ...
    def list_invalid_properties : Array(String)
      ...
      if _application_cryptogram = @application_cryptogram
        if max_length_error = OpenApi::PrimitiveValidator.max_length_error("application_cryptogram", _application_cryptogram.to_s.size, 5000)
          invalid_properties.push(max_length_error)
        end
      end
      if _application_preferred_name = @application_preferred_name
        if max_length_error = OpenApi::PrimitiveValidator.max_length_error("application_preferred_name", _application_preferred_name.to_s.size, 5000)
          invalid_properties.push(max_length_error)
        end
      end
      if _authorization_code = @authorization_code
        if max_length_error = OpenApi::PrimitiveValidator.max_length_error("authorization_code", _authorization_code.to_s.size, 5000)
          invalid_properties.push(max_length_error)
        end
      end
      if _authorization_response_code = @authorization_response_code
        if max_length_error = OpenApi::PrimitiveValidator.max_length_error("authorization_response_code", _authorization_response_code.to_s.size, 5000)
          invalid_properties.push(max_length_error)
        end
      end
      if _cardholder_verification_method = @cardholder_verification_method
        if max_length_error = OpenApi::PrimitiveValidator.max_length_error("cardholder_verification_method", _cardholder_verification_method.to_s.size, 5000)
          invalid_properties.push(max_length_error)
        end
      end
      if _dedicated_file_name = @dedicated_file_name
        if max_length_error = OpenApi::PrimitiveValidator.max_length_error("dedicated_file_name", _dedicated_file_name.to_s.size, 5000)
          invalid_properties.push(max_length_error)
        end
      end
      if _terminal_verification_results = @terminal_verification_results
        if max_length_error = OpenApi::PrimitiveValidator.max_length_error("terminal_verification_results", _terminal_verification_results.to_s.size, 5000)
          invalid_properties.push(max_length_error)
        end
      end
      if _transaction_status_information = @transaction_status_information
        if max_length_error = OpenApi::PrimitiveValidator.max_length_error("transaction_status_information", _transaction_status_information.to_s.size, 5000)
          invalid_properties.push(max_length_error)
        end
      end
