#  Stripe API
#
# The Stripe REST API. Please see https://stripe.com/docs/api for more details.
#
# The version of the OpenAPI document: 2024-06-20
# Contact: dev-platform@stripe.com
# Generated by: https://openapi-generator.tech
#

# Unit tests for Stripe::PostCheckoutSessionsRequest
# Automatically generated by openapi-generator (https://openapi-generator.tech)
# Please update as you see appropriate
require "../spec_helper"
require "../../src/stripe/models/post_checkout_sessions_request"

describe Stripe::PostCheckoutSessionsRequest do
  describe "test an instance of PostCheckoutSessionsRequest" do
    it "should create an instance of PostCheckoutSessionsRequest" do
      # instance = Stripe::PostCheckoutSessionsRequest.new
      # (instance).should be_a(Stripe::PostCheckoutSessionsRequest)
    end
  end

  describe "test attribute 'after_expiration'" do
    it "should work" do
      # instance = Stripe::PostCheckoutSessionsRequest.new
      # assertion here. ref: https://crystal-lang.org/reference/guides/testing.html
    end
  end

  describe "test attribute 'allow_promotion_codes'" do
    it "should work" do
      # instance = Stripe::PostCheckoutSessionsRequest.new
      # assertion here. ref: https://crystal-lang.org/reference/guides/testing.html
    end
  end

  describe "test attribute 'automatic_tax'" do
    it "should work" do
      # instance = Stripe::PostCheckoutSessionsRequest.new
      # assertion here. ref: https://crystal-lang.org/reference/guides/testing.html
    end
  end

  describe "test attribute 'billing_address_collection'" do
    it "should work" do
      # instance = Stripe::PostCheckoutSessionsRequest.new
      # assertion here. ref: https://crystal-lang.org/reference/guides/testing.html
      # validator = Stripe::PostCheckoutSessionsRequest::ENUM_VALIDATOR_FOR_BILLING_ADDRESS_COLLECTION
      # validator.allowable_values.each do |value|
      #   instance.billing_address_collection = value.as(String)
      # end
    end
  end

  describe "test attribute 'cancel_url'" do
    it "should work" do
      # instance = Stripe::PostCheckoutSessionsRequest.new
      # assertion here. ref: https://crystal-lang.org/reference/guides/testing.html
    end
  end

  describe "test attribute 'client_reference_id'" do
    it "should work" do
      # instance = Stripe::PostCheckoutSessionsRequest.new
      # assertion here. ref: https://crystal-lang.org/reference/guides/testing.html
    end
  end

  describe "test attribute 'consent_collection'" do
    it "should work" do
      # instance = Stripe::PostCheckoutSessionsRequest.new
      # assertion here. ref: https://crystal-lang.org/reference/guides/testing.html
    end
  end

  describe "test attribute 'currency'" do
    it "should work" do
      # instance = Stripe::PostCheckoutSessionsRequest.new
      # assertion here. ref: https://crystal-lang.org/reference/guides/testing.html
    end
  end

  describe "test attribute 'custom_fields'" do
    it "should work" do
      # instance = Stripe::PostCheckoutSessionsRequest.new
      # assertion here. ref: https://crystal-lang.org/reference/guides/testing.html
    end
  end

  describe "test attribute 'custom_text'" do
    it "should work" do
      # instance = Stripe::PostCheckoutSessionsRequest.new
      # assertion here. ref: https://crystal-lang.org/reference/guides/testing.html
    end
  end

  describe "test attribute 'customer'" do
    it "should work" do
      # instance = Stripe::PostCheckoutSessionsRequest.new
      # assertion here. ref: https://crystal-lang.org/reference/guides/testing.html
    end
  end

  describe "test attribute 'customer_creation'" do
    it "should work" do
      # instance = Stripe::PostCheckoutSessionsRequest.new
      # assertion here. ref: https://crystal-lang.org/reference/guides/testing.html
      # validator = Stripe::PostCheckoutSessionsRequest::ENUM_VALIDATOR_FOR_CUSTOMER_CREATION
      # validator.allowable_values.each do |value|
      #   instance.customer_creation = value.as(String)
      # end
    end
  end

  describe "test attribute 'customer_email'" do
    it "should work" do
      # instance = Stripe::PostCheckoutSessionsRequest.new
      # assertion here. ref: https://crystal-lang.org/reference/guides/testing.html
    end
  end

  describe "test attribute 'customer_update'" do
    it "should work" do
      # instance = Stripe::PostCheckoutSessionsRequest.new
      # assertion here. ref: https://crystal-lang.org/reference/guides/testing.html
    end
  end

  describe "test attribute 'discounts'" do
    it "should work" do
      # instance = Stripe::PostCheckoutSessionsRequest.new
      # assertion here. ref: https://crystal-lang.org/reference/guides/testing.html
    end
  end

  describe "test attribute 'expand'" do
    it "should work" do
      # instance = Stripe::PostCheckoutSessionsRequest.new
      # assertion here. ref: https://crystal-lang.org/reference/guides/testing.html
    end
  end

  describe "test attribute 'expires_at'" do
    it "should work" do
      # instance = Stripe::PostCheckoutSessionsRequest.new
      # assertion here. ref: https://crystal-lang.org/reference/guides/testing.html
    end
  end

  describe "test attribute 'invoice_creation'" do
    it "should work" do
      # instance = Stripe::PostCheckoutSessionsRequest.new
      # assertion here. ref: https://crystal-lang.org/reference/guides/testing.html
    end
  end

  describe "test attribute 'line_items'" do
    it "should work" do
      # instance = Stripe::PostCheckoutSessionsRequest.new
      # assertion here. ref: https://crystal-lang.org/reference/guides/testing.html
    end
  end

  describe "test attribute 'locale'" do
    it "should work" do
      # instance = Stripe::PostCheckoutSessionsRequest.new
      # assertion here. ref: https://crystal-lang.org/reference/guides/testing.html
      # validator = Stripe::PostCheckoutSessionsRequest::ENUM_VALIDATOR_FOR_LOCALE
      # validator.allowable_values.each do |value|
      #   instance.locale = value.as(String)
      # end
    end
  end

  describe "test attribute 'metadata'" do
    it "should work" do
      # instance = Stripe::PostCheckoutSessionsRequest.new
      # assertion here. ref: https://crystal-lang.org/reference/guides/testing.html
    end
  end

  describe "test attribute 'mode'" do
    it "should work" do
      # instance = Stripe::PostCheckoutSessionsRequest.new
      # assertion here. ref: https://crystal-lang.org/reference/guides/testing.html
      # validator = Stripe::PostCheckoutSessionsRequest::ENUM_VALIDATOR_FOR_MODE
      # validator.allowable_values.each do |value|
      #   instance.mode = value.as(String)
      # end
    end
  end

  describe "test attribute 'payment_intent_data'" do
    it "should work" do
      # instance = Stripe::PostCheckoutSessionsRequest.new
      # assertion here. ref: https://crystal-lang.org/reference/guides/testing.html
    end
  end

  describe "test attribute 'payment_method_collection'" do
    it "should work" do
      # instance = Stripe::PostCheckoutSessionsRequest.new
      # assertion here. ref: https://crystal-lang.org/reference/guides/testing.html
      # validator = Stripe::PostCheckoutSessionsRequest::ENUM_VALIDATOR_FOR_PAYMENT_METHOD_COLLECTION
      # validator.allowable_values.each do |value|
      #   instance.payment_method_collection = value.as(String)
      # end
    end
  end

  describe "test attribute 'payment_method_configuration'" do
    it "should work" do
      # instance = Stripe::PostCheckoutSessionsRequest.new
      # assertion here. ref: https://crystal-lang.org/reference/guides/testing.html
    end
  end

  describe "test attribute 'payment_method_data'" do
    it "should work" do
      # instance = Stripe::PostCheckoutSessionsRequest.new
      # assertion here. ref: https://crystal-lang.org/reference/guides/testing.html
    end
  end

  describe "test attribute 'payment_method_options'" do
    it "should work" do
      # instance = Stripe::PostCheckoutSessionsRequest.new
      # assertion here. ref: https://crystal-lang.org/reference/guides/testing.html
    end
  end

  describe "test attribute 'payment_method_types'" do
    it "should work" do
      # instance = Stripe::PostCheckoutSessionsRequest.new
      # assertion here. ref: https://crystal-lang.org/reference/guides/testing.html
      # validator = Stripe::PostCheckoutSessionsRequest::ENUM_VALIDATOR_FOR_PAYMENT_METHOD_TYPES
      # validator.allowable_values.each do |value|
      #   instance.payment_method_types = value.as(Array(String))
      # end
    end
  end

  describe "test attribute 'phone_number_collection'" do
    it "should work" do
      # instance = Stripe::PostCheckoutSessionsRequest.new
      # assertion here. ref: https://crystal-lang.org/reference/guides/testing.html
    end
  end

  describe "test attribute 'redirect_on_completion'" do
    it "should work" do
      # instance = Stripe::PostCheckoutSessionsRequest.new
      # assertion here. ref: https://crystal-lang.org/reference/guides/testing.html
      # validator = Stripe::PostCheckoutSessionsRequest::ENUM_VALIDATOR_FOR_REDIRECT_ON_COMPLETION
      # validator.allowable_values.each do |value|
      #   instance.redirect_on_completion = value.as(String)
      # end
    end
  end

  describe "test attribute 'return_url'" do
    it "should work" do
      # instance = Stripe::PostCheckoutSessionsRequest.new
      # assertion here. ref: https://crystal-lang.org/reference/guides/testing.html
    end
  end

  describe "test attribute 'saved_payment_method_options'" do
    it "should work" do
      # instance = Stripe::PostCheckoutSessionsRequest.new
      # assertion here. ref: https://crystal-lang.org/reference/guides/testing.html
    end
  end

  describe "test attribute 'setup_intent_data'" do
    it "should work" do
      # instance = Stripe::PostCheckoutSessionsRequest.new
      # assertion here. ref: https://crystal-lang.org/reference/guides/testing.html
    end
  end

  describe "test attribute 'shipping_address_collection'" do
    it "should work" do
      # instance = Stripe::PostCheckoutSessionsRequest.new
      # assertion here. ref: https://crystal-lang.org/reference/guides/testing.html
    end
  end

  describe "test attribute 'shipping_options'" do
    it "should work" do
      # instance = Stripe::PostCheckoutSessionsRequest.new
      # assertion here. ref: https://crystal-lang.org/reference/guides/testing.html
    end
  end

  describe "test attribute 'submit_type'" do
    it "should work" do
      # instance = Stripe::PostCheckoutSessionsRequest.new
      # assertion here. ref: https://crystal-lang.org/reference/guides/testing.html
      # validator = Stripe::PostCheckoutSessionsRequest::ENUM_VALIDATOR_FOR_SUBMIT_TYPE
      # validator.allowable_values.each do |value|
      #   instance.submit_type = value.as(String)
      # end
    end
  end

  describe "test attribute 'subscription_data'" do
    it "should work" do
      # instance = Stripe::PostCheckoutSessionsRequest.new
      # assertion here. ref: https://crystal-lang.org/reference/guides/testing.html
    end
  end

  describe "test attribute 'success_url'" do
    it "should work" do
      # instance = Stripe::PostCheckoutSessionsRequest.new
      # assertion here. ref: https://crystal-lang.org/reference/guides/testing.html
    end
  end

  describe "test attribute 'tax_id_collection'" do
    it "should work" do
      # instance = Stripe::PostCheckoutSessionsRequest.new
      # assertion here. ref: https://crystal-lang.org/reference/guides/testing.html
    end
  end

  describe "test attribute 'ui_mode'" do
    it "should work" do
      # instance = Stripe::PostCheckoutSessionsRequest.new
      # assertion here. ref: https://crystal-lang.org/reference/guides/testing.html
      # validator = Stripe::PostCheckoutSessionsRequest::ENUM_VALIDATOR_FOR_UI_MODE
      # validator.allowable_values.each do |value|
      #   instance.ui_mode = value.as(String)
      # end
    end
  end
end
