#  Stripe API
#
# The Stripe REST API. Please see https://stripe.com/docs/api for more details.
#
# The version of the OpenAPI document: 2022-08-01
# Contact: dev-platform@stripe.com
# Generated by: https://openapi-generator.tech
#

require "../spec_helper"

# Unit tests for Stripe::PostCustomersCustomerSubscriptionsSubscriptionExposedIdRequest
# Automatically generated by openapi-generator (https://openapi-generator.tech)
# Please update as you see appropriate
describe Stripe::PostCustomersCustomerSubscriptionsSubscriptionExposedIdRequest do
  describe "test an instance of PostCustomersCustomerSubscriptionsSubscriptionExposedIdRequest" do
    it "should create an instance of PostCustomersCustomerSubscriptionsSubscriptionExposedIdRequest" do
      # instance = Stripe::PostCustomersCustomerSubscriptionsSubscriptionExposedIdRequest.new
      # (instance).should be_a(Stripe::PostCustomersCustomerSubscriptionsSubscriptionExposedIdRequest)
    end
  end

  describe "test attribute 'add_invoice_items'" do
    it "should work" do
      # instance = Stripe::PostCustomersCustomerSubscriptionsSubscriptionExposedIdRequest.new
      # assertion here. ref: https://crystal-lang.org/reference/guides/testing.html
    end
  end

  describe "test attribute 'application_fee_percent'" do
    it "should work" do
      # instance = Stripe::PostCustomersCustomerSubscriptionsSubscriptionExposedIdRequest.new
      # assertion here. ref: https://crystal-lang.org/reference/guides/testing.html
    end
  end

  describe "test attribute 'automatic_tax'" do
    it "should work" do
      # instance = Stripe::PostCustomersCustomerSubscriptionsSubscriptionExposedIdRequest.new
      # assertion here. ref: https://crystal-lang.org/reference/guides/testing.html
    end
  end

  describe "test attribute 'billing_cycle_anchor'" do
    it "should work" do
      # instance = Stripe::PostCustomersCustomerSubscriptionsSubscriptionExposedIdRequest.new
      # assertion here. ref: https://crystal-lang.org/reference/guides/testing.html
      # validator = Stripe::PostCustomersCustomerSubscriptionsSubscriptionExposedIdRequest::ENUM_VALIDATOR_FOR_BILLING_CYCLE_ANCHOR
      # validator.allowable_values.each do |value|
      #   instance.billing_cycle_anchor = value.as(String)
      # end
    end
  end

  describe "test attribute 'billing_thresholds'" do
    it "should work" do
      # instance = Stripe::PostCustomersCustomerSubscriptionsSubscriptionExposedIdRequest.new
      # assertion here. ref: https://crystal-lang.org/reference/guides/testing.html
    end
  end

  describe "test attribute 'cancel_at'" do
    it "should work" do
      # instance = Stripe::PostCustomersCustomerSubscriptionsSubscriptionExposedIdRequest.new
      # assertion here. ref: https://crystal-lang.org/reference/guides/testing.html
    end
  end

  describe "test attribute 'cancel_at_period_end'" do
    it "should work" do
      # instance = Stripe::PostCustomersCustomerSubscriptionsSubscriptionExposedIdRequest.new
      # assertion here. ref: https://crystal-lang.org/reference/guides/testing.html
    end
  end

  describe "test attribute 'collection_method'" do
    it "should work" do
      # instance = Stripe::PostCustomersCustomerSubscriptionsSubscriptionExposedIdRequest.new
      # assertion here. ref: https://crystal-lang.org/reference/guides/testing.html
      # validator = Stripe::PostCustomersCustomerSubscriptionsSubscriptionExposedIdRequest::ENUM_VALIDATOR_FOR_COLLECTION_METHOD
      # validator.allowable_values.each do |value|
      #   instance.collection_method = value.as(String)
      # end
    end
  end

  describe "test attribute 'coupon'" do
    it "should work" do
      # instance = Stripe::PostCustomersCustomerSubscriptionsSubscriptionExposedIdRequest.new
      # assertion here. ref: https://crystal-lang.org/reference/guides/testing.html
    end
  end

  describe "test attribute 'days_until_due'" do
    it "should work" do
      # instance = Stripe::PostCustomersCustomerSubscriptionsSubscriptionExposedIdRequest.new
      # assertion here. ref: https://crystal-lang.org/reference/guides/testing.html
    end
  end

  describe "test attribute 'default_payment_method'" do
    it "should work" do
      # instance = Stripe::PostCustomersCustomerSubscriptionsSubscriptionExposedIdRequest.new
      # assertion here. ref: https://crystal-lang.org/reference/guides/testing.html
    end
  end

  describe "test attribute 'default_source'" do
    it "should work" do
      # instance = Stripe::PostCustomersCustomerSubscriptionsSubscriptionExposedIdRequest.new
      # assertion here. ref: https://crystal-lang.org/reference/guides/testing.html
    end
  end

  describe "test attribute 'default_tax_rates'" do
    it "should work" do
      # instance = Stripe::PostCustomersCustomerSubscriptionsSubscriptionExposedIdRequest.new
      # assertion here. ref: https://crystal-lang.org/reference/guides/testing.html
    end
  end

  describe "test attribute 'expand'" do
    it "should work" do
      # instance = Stripe::PostCustomersCustomerSubscriptionsSubscriptionExposedIdRequest.new
      # assertion here. ref: https://crystal-lang.org/reference/guides/testing.html
    end
  end

  describe "test attribute 'items'" do
    it "should work" do
      # instance = Stripe::PostCustomersCustomerSubscriptionsSubscriptionExposedIdRequest.new
      # assertion here. ref: https://crystal-lang.org/reference/guides/testing.html
    end
  end

  describe "test attribute 'metadata'" do
    it "should work" do
      # instance = Stripe::PostCustomersCustomerSubscriptionsSubscriptionExposedIdRequest.new
      # assertion here. ref: https://crystal-lang.org/reference/guides/testing.html
    end
  end

  describe "test attribute 'off_session'" do
    it "should work" do
      # instance = Stripe::PostCustomersCustomerSubscriptionsSubscriptionExposedIdRequest.new
      # assertion here. ref: https://crystal-lang.org/reference/guides/testing.html
    end
  end

  describe "test attribute 'pause_collection'" do
    it "should work" do
      # instance = Stripe::PostCustomersCustomerSubscriptionsSubscriptionExposedIdRequest.new
      # assertion here. ref: https://crystal-lang.org/reference/guides/testing.html
    end
  end

  describe "test attribute 'payment_behavior'" do
    it "should work" do
      # instance = Stripe::PostCustomersCustomerSubscriptionsSubscriptionExposedIdRequest.new
      # assertion here. ref: https://crystal-lang.org/reference/guides/testing.html
      # validator = Stripe::PostCustomersCustomerSubscriptionsSubscriptionExposedIdRequest::ENUM_VALIDATOR_FOR_PAYMENT_BEHAVIOR
      # validator.allowable_values.each do |value|
      #   instance.payment_behavior = value.as(String)
      # end
    end
  end

  describe "test attribute 'payment_settings'" do
    it "should work" do
      # instance = Stripe::PostCustomersCustomerSubscriptionsSubscriptionExposedIdRequest.new
      # assertion here. ref: https://crystal-lang.org/reference/guides/testing.html
    end
  end

  describe "test attribute 'pending_invoice_item_interval'" do
    it "should work" do
      # instance = Stripe::PostCustomersCustomerSubscriptionsSubscriptionExposedIdRequest.new
      # assertion here. ref: https://crystal-lang.org/reference/guides/testing.html
    end
  end

  describe "test attribute 'promotion_code'" do
    it "should work" do
      # instance = Stripe::PostCustomersCustomerSubscriptionsSubscriptionExposedIdRequest.new
      # assertion here. ref: https://crystal-lang.org/reference/guides/testing.html
    end
  end

  describe "test attribute 'proration_behavior'" do
    it "should work" do
      # instance = Stripe::PostCustomersCustomerSubscriptionsSubscriptionExposedIdRequest.new
      # assertion here. ref: https://crystal-lang.org/reference/guides/testing.html
      # validator = Stripe::PostCustomersCustomerSubscriptionsSubscriptionExposedIdRequest::ENUM_VALIDATOR_FOR_PRORATION_BEHAVIOR
      # validator.allowable_values.each do |value|
      #   instance.proration_behavior = value.as(String)
      # end
    end
  end

  describe "test attribute 'proration_date'" do
    it "should work" do
      # instance = Stripe::PostCustomersCustomerSubscriptionsSubscriptionExposedIdRequest.new
      # assertion here. ref: https://crystal-lang.org/reference/guides/testing.html
    end
  end

  describe "test attribute 'transfer_data'" do
    it "should work" do
      # instance = Stripe::PostCustomersCustomerSubscriptionsSubscriptionExposedIdRequest.new
      # assertion here. ref: https://crystal-lang.org/reference/guides/testing.html
    end
  end

  describe "test attribute 'trial_end'" do
    it "should work" do
      # instance = Stripe::PostCustomersCustomerSubscriptionsSubscriptionExposedIdRequest.new
      # assertion here. ref: https://crystal-lang.org/reference/guides/testing.html
    end
  end

  describe "test attribute 'trial_from_plan'" do
    it "should work" do
      # instance = Stripe::PostCustomersCustomerSubscriptionsSubscriptionExposedIdRequest.new
      # assertion here. ref: https://crystal-lang.org/reference/guides/testing.html
    end
  end
end
